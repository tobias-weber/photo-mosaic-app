...
public class ZipCollectionInstaller : ICollectionInstaller
{
    ...
    public async Task<bool> InstallAsync(TileCollectionConfig config, string dirPath)
    {
        ...
        return UnzipAndDeleteZip(zipPath, dirPath);
    }
    ...
    private bool UnzipAndDeleteZip(string zipPath, string dirPath)
    {
        var isSuccess = true;
        try
        {
            ...
            foreach (var entry in archive.Entries)
            {
                ...
                entry.ExtractToFile(extractedFilePath, overwrite: true); // this might drop files if they do not have unique names
            }
        }
        catch (Exception ex)
        {
